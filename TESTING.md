...
| 12 | The confirmation email is not rounding the values to 2 decimal places and the contact.email value is not displaying | I solved this by adding `float:2` to the values in the confirmation email body file. UPDATE: This error was actually being caused in part to solved bug 16, and not using decimal on some values. Once these were changed there was no need to have the float on the values for the email and so these have been removed. | ![Confirmation Email Bug](documentation/bugs/confirmation-email.png) |
...
| 16 | Once a user completes payment the order is added to their order history, the webhook handler checks to see if an order has been saved, and if not creates the order in the database (this is a fail-safe to prevent orders not being created in the database if a user leaves the checkout page during processing) and sends a confirmation email. However once I sorted the issue with the webhooks not working, I am getting an error where two orders are created and added to the users profile under order history. The user only receives one confirmation email, which matches the latest addition to the database. | After double checking through my code I was unable to see a cause of what could be doing this, although I did know that the issue lay somewhere in the webhook_hander, so I utilised Tutor support to try and rectify this issue. We were able to deduce that there was an issue in the handle_payment_intent_succeeded function where the code was not seeming to run the while loop section (picked up by adding print statments within the while loop), however we know that the webhook is creating an order, as it is sending the confirmation email with an order ID which can be found in the database. So the next question became where was the second duplicate order with a different ID coming from? After many hours (and even more print statements) the solution to the problem was a simple one - I was missing order.stripe_pid = pid underneath where I was declaring the pid in the checkout view. | ![Solved Bug 16](documentation/bugs/solved-bug-16.jpg) |
| 17 | 400 Error on deployed site when deploying with heroku | Originally I had included `https://seaside-sewing-5d9a182dabe3.herokuapp.com/` in the allowed hosts section in settings.py - this needed to be `seaside-sewing-5d9a182dabe3.herokuapp.com` | |
